...
def _extract_result_block(payload: Any) -> Optional[Dict[str, Any]]:
    """Return the nested Avast result block when present."""
    if not isinstance(payload, dict):
        return None

    for key in ("avast", "avast-core", "avast_core"):
...
def _build_url() -> str:
    host = os.getenv("AVAST_HOST", DEFAULT_HOST)
    port = int(os.getenv("AVAST_PORT", str(DEFAULT_PORT)))
    return f"http://{host}:{port}/scan"


def run(file_path: str):
    """Scan a file using the Avast web service."""
    ...
    url = _build_url()
    timeout = float(os.getenv("AVAST_TIMEOUT", str(DEFAULT_TIMEOUT)))
    ...
    for key in ("engine", "version"):
        if isinstance(result_block, dict) and result_block.get(key):
            engine_version = str(result_block.get(key))
            break
