...
## Project snapshot
- Multi-engine malware scanner: FastAPI API (`app/main.py`), Celery workers (`app/workers/tasks.py`), AV engines (ClamAV, YARA, Windows Defender), storage abstraction (`app/services/storage.py`), and Next.js UI (`ui/web`).
...
## Run the stack
- Preferred: `docker compose up --build` from repo root. Services: API (8000), frontend (3000), Postgres (55432 in compose), Redis (6380), ClamAV (3310), Windows Defender (3993), MinIO (9000/9001).
- Next.js UI: `http://localhost:3000` after compose is up. API docs: `http://localhost:8000/docs`.
...
- Next.js UI: from `ui/web`, run `npm install` then `npm run dev` (configure via `.env.local` / env vars).
- Without engine services, tests stub ClamAV; real scans need the engines running.

## Testing
- Full suite: `pytest`. Uses sqlite + in-memory Celery/Redis via `tests/conftest.py`; no external services required.
...
- Python/TypeScript code: prefer type hints, small helpers, and minimal but clear logging/errors. Preserve existing status strings and JSON shapes.
